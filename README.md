...
Optimal portfolio strategies for a large universe of assets

## Data

Daily OLHCV data over 10 years

SPY: benchmark from Yahoo Finance (not accurate as its dynamically changed but none the less can be included)
Sectors: Bloomberg data from 100+ macro indecies
Assets: Yahoo Finance data for all current 500 SPY constituients

## Methodology

### Methodology HSP:

Common drivers are selected as the top \( K \) (hyperparameter 1) with the highest common correlation with respect to all portfolio constituents, using correlation time series of daily returns and a time window \( W*{CD} \) (hyperparameter 2). With this common driver selection as inputs and one portfolio constituent as output, we train several feed‑forward networks for the prediction task. We use a time window \( W*{NN} \) (hyperparameter 3) for many architectures (varying numbers of layers and neurons) and select the most accurate one. We repeat this process for the rest of the portfolio constituents using the same common drivers.
