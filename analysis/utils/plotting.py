...
def plot_performance_summary(
    ...
    fig.suptitle("Strategy Performance Summary", fontweight="bold", y=1.01)
    fig.tight_layout()
    return _save(fig, save_path)
...
def plot_weights_heatmap(
    ...
    ax.set_yticklabels(weights_df.columns, fontsize=9)
...
def plot_regime_detection(
    ...
    ax2.set_yticklabels(list(regime_to_int.keys()))
        ...
        for j in range(len(pivot.columns)):
            ...
            if not np.isnan(val):
                ax.text(j, i, f"{val*100:.1f}%", ha="center", va="center",
                        fontsize=9, color="black" if abs(val) < 0.12 else "white")
        yr = pivot.index[i]
...
def plot_risk_return_scatter(
    ...
    for sr in [0, 0.5, 1.0, 1.5, 2.0]:
        ...
        ax.text(vol_range[mid], ret_line[mid] + 0.004,
                f"SR={sr}", fontsize=10, color="grey", va="bottom")
